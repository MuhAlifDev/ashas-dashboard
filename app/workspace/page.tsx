...
import type { Drawing, ToolCategory } from "../_lib/types";

type WorkspaceMode = "boards" | "tools";

export default function WorkspacePage() {
  const { data, addDrawing, updateDrawing, deleteDrawing } = useDashboard();
  const { drawings, tools } = data;

  const [mode, setMode] = useState<WorkspaceMode>("boards");
  const [activeId, setActiveId] = useState<string | null>(null);
  const [filterCategory, setFilterCategory] = useState<ToolCategory | null>(
    null,
  );
  ...
  const toolCountByCategory = CATEGORY_ORDER.reduce<
  ...
  return (
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="flex w-56 shrink-0 flex-col border-r border-slate-gray/20 bg-navy-dark text-off-white">
        {/* Tabs */}
        <div className="flex border-b border-slate-gray/20 shrink-0">
          <button
            ...
            className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider transition-colors border-b-2 ${
              mode === "boards"
                ...
                : "text-slate-gray border-transparent hover:text-off-white"
            }`}
          >
            Papan
          </button>
        ...
        {mode === "boards" && (
          <>
            <div className="border-b border-slate-gray/20 px-4 py-3 shrink-0">
              <button
                onClick={createBoard}
                    ...
                    {renamingId === d.id ? (
                      ...
                      <>
                        <div className="flex items-center gap-2">
                          <svg
                            className="h-3 w-3 shrink-0 text-slate-gray"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            strokeWidth="2"
            ...
            <div className="border-t border-slate-gray/20 px-4 py-3 shrink-0">
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider">
                ...
                {saveStatus === "idle" && (
                  <>
                    <span className="h-1.5 w-1.5 rounded-full bg-slate-gray/50" />
                    <span className="text-slate-gray">Auto-save aktif</span>
                  </>
                )}
              </div>
            </div>
        ...
        {mode === "tools" && (
          <div className="flex-1 overflow-y-auto">
            ...
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-2.5 text-xs font-bold transition-colors ${
                filterCategory === null
                  ...
                  : "text-slate-gray hover:text-off-white hover:bg-white/5"
              }`}
            >
              <span>Semua</span>
              <span
            ...
            {CATEGORY_ORDER.map((cat) => {
              const count = toolCountByCategory[cat];
              return (
                <button
                  key={cat}
        ...
        {mode === "boards" ? (
          activeDrawing ? (
            <ExcalidrawBoard
              ...
              onSave={(drawingData) =>
                handleSave(activeDrawing.id, drawingData)
              }
            />
          ) : (
            <div className="flex flex-1 items-center justify-center bg-off-white">
              <div className="text-center">
                ...
                <p className="text-sm text-slate-gray mt-2 max-w-xs">
                  Gunakan whiteboard untuk wireframe, diagram, mind map, atau
                  catatan visual apa saja.
                </p>
                <button
                  onClick={createBoard}
          ...
          <ToolsPanel filterCategory={filterCategory} />
        )}
      </main>
    </div>
  );
